...
import interfaces.BaseExcelReader;
import interfaces.BaseExcelWriter;
import interfaces.TitleCreatedExcelWriter;
import interfaces.TitleLoadedRowReader;
...
public class ExcelUtil {
    ...
    private static File init(String fileName) throws UnsupportedEncodingException {
        ClassLoader classLoader = ClassLoader.getSystemClassLoader();
        URL resource = classLoader.getResource(fileName);
        Objects.requireNonNull(resource);
        String fileNameTransform = java.net.URLDecoder.decode(resource.getPath(), "utf-8");
                ...
                for (Cell cell : row) {
                    ...
                    if (StringUtils.isBlank(cellContent)) {
                        ...
                        if (null != contentMapCol.get(cellContent)) {
                            throw new RuntimeException("首列名作为键值重复 : " + cellContent);
                        }
